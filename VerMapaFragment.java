    ...
    private OnMapReadyCallback callback = new OnMapReadyCallback() {
        ...
        public void onMapReady(GoogleMap googleMap) {
            ...
            if (bundle != null) {
                ...
                String sedeNombre = bundle.getString("sedeNombre");

                // Verificar si las coordenadas no son nulas
                if (latitud != null && longitud != null) {
                    double lat = Double.parseDouble(latitud);
                    double lon = Double.parseDouble(longitud);
                    LatLng ubicacion = new LatLng(lat, lon);

                    // Mover la cámara a la ubicación de la sede
                    googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(ubicacion, 12f));

                    // Añadir marcador en la ubicación recibida
                    googleMap.addMarker(new MarkerOptions()
                            .position(ubicacion)
                            .title(sedeNombre)  // Usar el nombre de la sede
                            .icon(iconoPersonalizado));
                }
            }
